...
class ProcessWithGeminiTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_openai_text_provider_uses_gemini_credentials(self):
        ...
        class RecordingGenerator:
            ...
            def __init__(self, model_name, api_key, prompt, base_url):
                RecordingGenerator.init_kwargs = {
                    "model_name": model_name,
                    "api_key": api_key,
                    "prompt": prompt,
                    "base_url": base_url,
                }
                RecordingGenerator.instance = self
        ...
        class RecordingProcessor:
            ...
            def __init__(self, *args, **kwargs):
                RecordingProcessor.init_kwargs = kwargs
                self.generator = None

            def process_frames(self, frame_content_list):
                RecordingProcessor.generator_instance = self.generator
                return frame_content_list
        ...
        with patch.dict(
            config.app,
            {
                ...
                "vision_gemini_base_url": "https://vision.example.com",
                "vision_analysis_prompt": "describe",
                "text_llm_provider": " Gemini(OpenAI) ",
                "text_gemini_api_key": "text-key",
                "text_gemini_model_name": "gemini-pro",
                "text_gemini_base_url": "https://text.example.com",
            },
            ...
            "app.utils.gemini_openai_analyzer.GeminiOpenAIAnalyzer",
            return_value=DummyAnalyzer(),
        ), patch(
            "app.utils.script_generator.GeminiOpenAIGenerator",
            RecordingGenerator,
        ), patch(
            "app.services.script_service.ScriptProcessor",
            RecordingProcessor,
        ...
        self.assertEqual(len(result), 1)
        self.assertEqual(RecordingProcessor.init_kwargs["model_name"], "gemini-pro")
        self.assertEqual(RecordingProcessor.init_kwargs["api_key"], "text-key")
        self.assertEqual(RecordingProcessor.init_kwargs["base_url"], "https://text.example.com")
        self.assertEqual(RecordingProcessor.init_kwargs["prompt"], "custom")
        self.assertEqual(RecordingGenerator.init_kwargs["model_name"], "gemini-pro")
        self.assertEqual(RecordingGenerator.init_kwargs["api_key"], "text-key")
        self.assertEqual(RecordingGenerator.init_kwargs["base_url"], "https://text.example.com")
        self.assertEqual(RecordingGenerator.init_kwargs["prompt"], "custom")
        self.assertIs(RecordingProcessor.generator_instance, RecordingGenerator.instance)

    async def test_blank_text_provider_falls_back_to_gemini(self):
        generator = ScriptGenerator()
        fake_frames = [
            "/tmp/frame_000001_00:00:00,000.jpg",
            "/tmp/frame_000002_00:00:05,000.jpg",
        ...
        self.assertIsNotNone(RecordingProcessor.init_kwargs)
        self.assertEqual(RecordingProcessor.init_kwargs["model_name"], "gemini-pro")
        self.assertEqual(len(result), 1)
